...
export const friendlyError = error => friendly[error?.code] || error?.message || "Request failed. Please try again.";
export function buildHeaders({body,projectId,admin=false,idempotencyKey,userApiKey=userKey,adminApiKey=adminKey}={}) { const headers={};if(body!==undefined)headers["Content-Type"]="application/json";if(userApiKey)headers["X-User-API-Key"]=userApiKey;if(admin&&adminApiKey)headers["X-Admin-API-Key"]=adminApiKey;if(projectId)headers["X-Project-ID"]=projectId;if(idempotencyKey)headers["Idempotency-Key"]=idempotencyKey;return headers; }
export const jobRoute = kind => `/api/jobs/${kind}/generate`;

export async function api(path, { method = "GET", body, projectId, admin = false, idempotencyKey } = {}) {
  const headers = buildHeaders({body,projectId,admin,idempotencyKey});
