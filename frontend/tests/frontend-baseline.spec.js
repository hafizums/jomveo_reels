...
async function openGenerateWithProject(page) {
...
test("dashboard route loads the workspace overview", async ({ page }) => {
  await page.goto("/dashboard");
...
test("generate route and dashboard navigation expose generator tabs", async ({ page }) => {
  await page.goto("/dashboard");
  await page.getByRole("link", { name: "Generate" }).click();
  await expect(page).toHaveURL(/\/generate$/);
...
test("project selection persists across reload", async ({ page }) => {
  await page.goto("/dashboard"); await page.getByLabel("Project").selectOption(project.id);
...
test("creates and selects a project", async ({ page }) => {
  ...
  await page.goto("/dashboard"); await page.getByPlaceholder("New project name").fill("New Project"); await page.getByRole("button", { name: "Create project" }).click();
...
test("queue button sends project and idempotency headers", async ({ page }) => {
  ...
  await page.goto("/generate"); await page.getByRole("button", { name: "Queue script job" }).click();
...
test("project jobs route opens routed job details and direct asset link", async ({ page }) => {
  await page.goto(`/projects/${project.id}/jobs`); await page.getByRole("button", { name: /script.generate/ }).click();
  await expect(page).toHaveURL(/\/jobs\/job_1$/);
  await expect(page.getByText("Attempt 1/3")).toBeVisible();
  await expect(page.getByText("Progress 1/1")).toBeVisible();
...
test("polls nonterminal jobs and stops after completion", async ({ page }) => {
  ...
  await page.goto("/dashboard"); await expect(page.getByText("queued", { exact: true })).toBeVisible();
...
test("synchronous script generation still renders its result", async ({ page }) => {
  ...
  await page.goto("/generate"); await page.getByRole("button", { name: "Generate script" }).click();
...
test("synchronous music generation still renders audio output", async ({ page }) => {
  ...
  await page.goto("/generate"); await page.getByRole("tab", { name: "Background Music" }).click(); await page.getByRole("button", { name: "Generate background music" }).click();
...
test("synchronous art generation still renders image output", async ({ page }) => {
  ...
  await page.goto("/generate"); await page.getByRole("tab", { name: "Art Style" }).click(); await page.getByRole("button", { name: "Generate art-style image" }).click();
...
test("persistent controls use exact routes, project, and idempotency headers", async ({ page }) => {
  ...
  await mockApi(page, handlers); await openGenerateWithProject(page);
  ...
  await expect(page.getByRole("heading", { name: "Projects, jobs & temporary assets" })).toBeVisible();
  expect(jobRefreshes).toBeGreaterThanOrEqual(2);
...
test("empty voiceover text blocks persistent request with a friendly message", async ({ page }) => {
  ...
  await openGenerateWithProject(page); await page.getByRole("tab", { name: "Voiceover" }).click(); await page.getByRole("button", { name: "Queue voiceover job" }).click();
...
test("scene sequence uses slash route after synchronous script prerequisite", async ({ page }) => {
  ...
  await openGenerateWithProject(page); await page.getByRole("button", { name: "Generate script" }).click(); await page.getByRole("tab", { name: "Art Style" }).click(); await page.getByRole("button", { name: "Queue scene sequence job" }).click();
...
test("missing project and prerequisites show friendly errors without wrong requests", async ({ page }) => {
  ...
  await mockApi(page, { "GET /api/projects": route => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ projects: [], count: 0 }) }), "POST /api/jobs/scene-animations/generate": route => { queueCalls += 1; return route.abort(); }, "POST /api/jobs/videos/generate": route => { queueCalls += 1; return route.abort(); } });
  await page.goto("/generate"); await page.getByRole("button", { name: "Queue script job" }).click();
  ...
  await page.getByRole("link", { name: "Dashboard" }).click(); await expect(page.getByLabel("Project")).toHaveValue(project.id); await page.getByRole("link", { name: "Generate" }).click(); await page.getByRole("tab", { name: "Animate Scenes" }).click(); await page.getByRole("button", { name: "Queue animation job" }).click();
...
test("project assets and billing routes render existing cards", async ({ page }) => {
  await page.goto(`/projects/${project.id}/assets`);
  await expect(page.getByRole("heading", { name: "Project assets", level: 1 })).toBeVisible();
  await expect(page.getByRole("link", { name: "Open / Download" })).toHaveAttribute("href", asset.url);
  await page.goto(`/projects/${project.id}/billing`);
  await expect(page.getByRole("heading", { name: "Billing and quotas" })).toBeVisible();
  await expect(page.getByText("900", { exact: true })).toBeVisible();
  await expect(page.getByText(/top_up/)).toBeVisible();
});
